...
PHYSACDEF void SetPhysicsTimeStep(float delta);                                                            // Sets physics fixed time step in milliseconds. 1.666666 by default
...
static float baseTime = 0.0f;                               // Offset time for MONOTONIC clock
static float startTime = 0.0f;                              // Start time in milliseconds
static float deltaTime = 1.0 / 60.0 / 10.0 * 1000;         // Delta time used for physics steps, in milliseconds
static float currentTime = 0.0;                            // Current time in milliseconds
static uint64_t frequency = 0;                              // Hi-res clock frequency

static float accumulator = 0.0f;                            // Physics time step delta time accumulator
...
uint64_t GetTimeCount(void);                                                                         // Get hi-res MONOTONIC time measure in mseconds
float GetCurrTime(void);                                                                         // Get current time measure in milliseconds
...
PHYSACDEF void InitPhysics(void)
{
    ...
    xTaskCreate(PhysicsLoop, "PhysicsLoop", 4 * 1024, NULL, 1, NULL);
...
PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(Vector2 pos, float width, float height, float density)
{
    ...
    if (newId != -1) {
        ...
        newBody->enabled = true;
        newBody->position = pos;
        newBody->velocity = (Vector2) {
            0.0f, 0.0f
        };
        newBody->force = (Vector2) {
            0.0f, 0.0f
        };
        newBody->angularVelocity = 0.0f;
        newBody->torque = 0.0f;
        ...
        newBody->shape.transform = Mat2Radians(0.0f);
        newBody->shape.vertexData = CreateRectanglePolygon(pos, (Vector2) {
            width, height
        });
...
static PolygonData CreateRectanglePolygon(Vector2 pos, Vector2 size)
{
    ...
    // Calculate polygon vertices positions
    data.positions[0] = (Vector2) {
        pos.x + size.x / 2, pos.y - size.y / 2
    };
    data.positions[1] = (Vector2) {
        pos.x + size.x / 2, pos.y + size.y / 2
    };
    data.positions[2] = (Vector2) {
        pos.x - size.x / 2, pos.y + size.y / 2
    };
    data.positions[3] = (Vector2) {
        pos.x - size.x / 2, pos.y - size.y / 2
    };
...
PHYSACDEF void RunPhysicsStep(void)
{
    ...
    const float delta = currentTime - startTime;

    // Store the time elapsed since the last frame began
    accumulator += 1.0f * delta;
...
PHYSACDEF void SetPhysicsTimeStep(float delta)
{
    deltaTime = delta;
}
...
static void SolveCircleToCircle(PhysicsManifold manifold)
{
    ...
    if (distance == 0.0f) {
        ...
        manifold->contacts[0] = (Vector2) {
            manifold->normal.x *bodyA->shape.radius + bodyA->position.x, manifold->normal.y *bodyA->shape.radius + bodyA->position.y
        };
    }
...
static void SolvePolygonToPolygon(PhysicsManifold manifold)
{
    ...
    }, negSide, &incidentFace[0], &incidentFace[1]) < 2)
    return;
    ...
    manifold->normal = (flip ? (Vector2) {
        -refFaceNormal.x, -refFaceNormal.y
        } : refFaceNormal);
...
static void InitializePhysicsManifolds(PhysicsManifold manifold)
{
    ...
    for (int i = 0; i < manifold->contactsCount; i++) {
        ...
        gravityForce.x *deltaTime / 1000, gravityForce.y *deltaTime / 1000
    }) + PHYSAC_EPSILON))
    ...
    for (int i = 0; i < manifold->contactsCount; i++) {
        ...
        if (bodyA->enabled) {
            ...
            if (!bodyA->freezeOrient)
                bodyA->angularVelocity += bodyA->inverseInertia * MathCrossVector2(radiusA, (Vector2) {
                -impulseV.x, -impulseV.y
                });
        }
        ...
        if (absImpulseTangent < impulse * manifold->staticFriction)
            ...
            tangent.x *impulseTangent, tangent.y *impulseTangent
        };
        else
            tangentImpulse = (Vector2) {
            tangent.x * -impulse *manifold->dynamicFriction, tangent.y * -impulse *manifold->dynamicFriction
        };
...
static float FindAxisLeastPenetration(int *faceIndex, PhysicsShape shapeA, PhysicsShape shapeB)
{
    ...
    for (int i = 0; i < dataA.vertexCount; i++) {
        ...
        Vector2 support = GetSupport(shapeB, (Vector2) {
            -normal.x, -normal.y
            });
...
#if defined(CONFIG_IDF_TARGET_ESP32S3)
    // ESP32 returns time in microseconds (us), convert to nanoseconds (ns)
...
{
    return (float)(GetTimeCount() - baseTime) / frequency * 1000.0f;
}
...
{
    return (Mat2) {
        matrix.m00, matrix.m10, matrix.m01, matrix.m11
    };
}
...
{
    return (Vector2) {
        matrix.m00 *vector.x + matrix.m01 *vector.y, matrix.m10 *vector.x + matrix.m11 *vector.y
    };
}
